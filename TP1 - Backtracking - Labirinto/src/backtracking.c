...
int movimenta_estudantePoder(Mapa mapa, estudante aluno, coordenadas dimensao, int *cont, pilha* stack, int *moveX, int *moveY, coordenadas posicao,int poderes){
	...
	(*cont)++;
	return movimenta_estudantePoder(mapa, aluno, dimensao, cont, stack, moveX, moveY, posicao,poderes);
}
...
void ExploraLabirintoPoder(Mapa mapa, int linhas, int colunas, estudante aluno, int poderes){
	...
	if(movimenta_estudantePoder(mapa, aluno, dimensao, &chamadasRecursivas, stack, moveX, moveY, posicao,poderes) == -1){
		printf("\nO estudante se moveu %d vezes e percebeu que o labirinto não tem saída!\n", chamadasRecursivas);
		destroiPilha(stack);
		return;
